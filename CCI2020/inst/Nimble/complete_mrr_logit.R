model{
    ...
    for(i in 1:nrecovery){
      Prob[nrecapture + i] <- drecov(delta[nrecapture + i], 
                                     phi[nrecapture + i,1:delta[nrecapture + i]], 
                                     p, lambda)
    }

    ## 3) Evasions
    for(i in 1:(nrelease - nrecapture - nrecovery)){
      Prob[nrecapture + nrecovery + i] <- devade(delta[nrecapture + nrecovery + i], 
                                                 phi[nrecapture + nrecovery + i,1:delta[nrecapture + nrecovery + i]], 
                                                 p, lambda)
    }
